...
func main() {
	...
	for scanner.Scan() {
		...
		// Filter by tags
		if len(config.ignoredTags) > 0 && tagInTagsRegex(tag, config.ignoredTags) {
			continue
		}
		if len(config.tags) > 0 && !tagInTagsRegex(tag, config.tags) {
			continue
		}
